...
def  read_daizhi(filepath=None, seps=','):
    ...
    下载地址为：
    :param filepath
    :return: 与read——wiki2() 一样的结果
    """

    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    paragraphs = []
    for line in tqdm(lines , ncols= 80, desc=  '## read Daizhi') :
        if len(line.split(seps)) < 2 :
            continue
        line = line.strip()
        paragraphs.append([])

        for z in line.split(seps):
            paragraphs[-1].append(z)
...
class LoadBertPretrainingDataset(object):
    ...
    def get_format_data(self, file_path):
        ...
        elif self.data_name == 'Daizhi':
            print("ok")
            print(file_path)
            print(self.seps)
            return read_daizhi(file_path, self.seps)
        else:
